...
Generate mel spectograms for the INA set
    ...
    for line in lab: 
        if len(line.split(' ')) > 3:
            ...
            if 'sil' not in ph:
                ...
                frames.append([time2frame(start), time2frame(end)])
                scores.append(float(elements[-1]))
                ph_seq.append([ph, wd, str(PHONEME_DICT[ph]) ])
                      
    
    return np.array(time).astype(float), np.array(frames).astype(int), np.array(scores).astype(float), ph_seq
...
#this computes the mel spectogram (to dB) 
    ...
    if name in args.test_sentences:
        ...
        plt.savefig('/share/mini1/res/t/asr/call/childread-nl/its/aa/ad_lstm/task/melspec_'+str(args.num_filters)+'_'+name+'.png', dpi = 300)
...
def process(file_lists, lab_lists, dataset,speaker2index, f_h5, args ):
    ...
    for fname in file_lists:
        ...
        print(f"h5_path {h5_path} n_frames {mel.shape[0]} phone_segments {len(ph_seq)} speaker {speaker} idx {speaker_idx}",flush=True)
...
#        ###debug
#        
#        print(f"This is example {h5_path}")
#        print("mel")
#        print(f_h5[f'{h5_path}/mel'])
#        print("speaker")
#        print(f_h5[f'{h5_path}/speaker'])
#        print("time_info")
#        print(f_h5[f'{h5_path}/time_info'])
#        print("frame_info")
#        print(f_h5[f'{h5_path}/frame_info'])
#        print("scores_info")
#        print(f_h5[f'{h5_path}/scores_info'])
#        print("ph_seq")
#        print(f_h5[f'{h5_path}/ph_seq'])
#        
#        ###debug
        
        data_length.append( (h5_path, mel.shape[0])  )
...
def main():
	...
	parser.add_argument('--test_sentences', help='Sentences used for debug',default=['ina-agnt51199499hmon_agnt5136f98d80', 'ina-ndpt51274859hmdl_ndpt5110f93d96', 'ina-ndpt52988982hmta_ndpt5217m94d96', 'ina-zrnk51767756hmmz_zrnk5104m97d94'])
	parser.add_argument('--f', default=False, action='store_true')
    
    #sentences for debug
	#ina-agnt51199499hmon_agnt5136f98d80 199499
	#ina-ndpt51274859hmdl_ndpt5110f93d96 274859
	#ina-ndpt52988982hmta_ndpt5217m94d96 988982
	#ina-zrnk51767756hmmz_zrnk5104m97d94 767756
