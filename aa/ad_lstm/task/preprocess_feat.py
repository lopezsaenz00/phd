...
# -*- coding: utf-8 -*-
"""
Modified on Mon March 15 12:58:00 2021
From Mingjie Chen

@author: Jose Antonio Lopez @ The University of Sheffield

Generate PLP features for the INA set
...
import load_txt
import iohandles, iomessages
import textgrid
import codecs, unicodedata
import math

from HTKfile import *
HTK   = '/htk64/v3.4.1/x86_64/bin/' 
modulename = 'preprocess_feat'
...
#L2ARCTIC      
#PHONEME_DICT = { 'aa': 0, 'ae': 1, 'ah': 2, 'ao': 3, 'aw': 4, 'ax': 5, 'ay': 6, 'b': 7, 'ch': 8, 'd': 9,
#                    'dh': 10, 'eh': 11, 'er': 12, 'ey' : 13, 'f' : 14, 'g' : 15, 'hh' : 16, 'ih' : 17, 'iy' : 18, 'jh' : 19, 
#                    'k' : 20, 'l' : 21 , 'm' : 22, 'n' : 23, 'ng' : 24, 'ow' : 25, 'oy' : 26, 'p' : 27, 'r' : 28, 's' : 29, 'sh' : 30, 
#                    't' : 31, 'th' : 32, 'uh' : 33, 'uw' : 34, 'v' : 35, 'w' : 36, 'y' : 37, 'z' : 38, 'zh': 39, 'sil' : 40, 'sp' : 40, '' : 40, 'spn' : 40, }
#                    #we dont care about silence
...
#################################################################################
def lab2frame(n, win_time = 0.025, hop_time = 0.01, samplerate = 16000):
#from : http://www1.icsi.berkeley.edu/Speech/faq/time-to-frames.html
#win=25ms, hop=10ms and samplerate=8000Hz.
#Then the window is 200 samples
#the hop is 80 samples.
#example. 3.5 hours at 16k are 201600000 samples
	...
	return time2frame( n*1e-7 ,win_time , hop_time, samplerate )
...
#################################################################################
def load_wav_scp(wavpath):
#loads it as a tuple for the loop in process
    lines = load_txt.load_txtfile_aslist(wavpath)
...
def extract_labels(labfile):
    ...
    frame = [] #the number frame information of each phone segment
    ...
    for line in lab: 
        if len(line.split(' ')) > 3:
            ...
            nf = time2frame(end-start)
            ph = elements[2][elements[2].rfind('_')+1:]
            wd = elements[2][:elements[2].rfind('_')]
            
            if 'sil' not in ph:
                time.append([start, end])
                frame.append([time2frame(start), time2frame(end)])
                scores.append(float(elements[-1]))
                ph_seq.append([ph, wd, str(PHONEME_DICT[ph]) ])
            
    return np.array(time).astype(float), np.array(frame).astype(int), np.array(scores).astype(float), ph_seq
...
#################################################################################
def load_mlf(filepath):
#loads the annotations as a dict of lists for easy lookup.

    lines = load_txt.load_txtfile_aslist(filepath)
    segments = []
    mlf_dict = {}
    ...
    for line in lines:
        ...
        if '.lab' in linesplit[0]:
            if segments:
                mlf_dict[name] = segments
        
            name=linesplit[0].replace('*/','').replace('.lab','').strip('\"')
            segments = []
            
        if len(linesplit) > 2 and not(linesplit[-1] == 'sil'):
            segments.append(linesplit)
...
###############################################################################################
def resample(audio):
    #use sox to resample auto to 16k
    #submit the wav file and the location of the tmp folder 
    #WDIR
    
    ##Resample and save it in the tmp folder
    FEA = os.path.join(WDIR,os.path.basename(audio))
    #arrange the resampling
    cmd = [ 'sox', audio, '-r', '16k', FEA ]
    
    # Run script
    iohandles.executescript(cmd, output = False)
    
    return FEA
    
    
###############################################################################################
def get_fbank(fname, args, newname):
    #use sox to resample auto to 16k
    #submit the wav file and the location of the tmp folder 
    #WDIR
...
###############################################################################################   
def extract_features(audio, newname, args):
    '''runs HTK script, HCopy, to extract features from audio file. Check 
    config.hcopy.wav file in lib/ to change audio and fea  characteristcs'''
    
    # get features
    # -----------------------------------------------------------------------------
    # Setup names of intermediate files
    # -----------------------------------------------------------------------------
    FEA = os.path.join(WDIR, newname.replace( '.wav', '.'+ FEATTYPE))
    cfg = args.hconfig
        
    # check sw
    if not os.path.isfile(os.path.join(HTK,'HCopy')):
      iomessages.fatal(modulename, 'HCopy not found')

    # check conf file
    if not os.path.isfile(cfg):
      iomessages.fatal(modulename, cfg + ' not found')

    # Log
    iomessages.shortinfo(modulename,'using ' + cfg)

    # Do the coding - to features
    cmd = [os.path.join(HTK,'HCopy'),'-T','7','-A','-D','-V','-C', cfg, audio, FEA]

    # Run script
    iohandles.executescript(cmd, output = False);
    
    #read features as numpy array
    features = read_htk(FEA)
    os.remove(FEA)
...
#this method loads a single htk file. It requires the name(path) to the file
# and it return the numpy array with the shape (frames, feature dimension)

	#first we create and HTKfile object (python) that can read the file
        htk_reader = HTKFile()
        #now the object reads the file
        htk_reader.load(file_name)
        
        return np.array(htk_reader.data)
   
    
###############################################################################################
def get_melspectrogram(fname, args, newname):
#this computes the mel spectogram (to dB) 
    ...
    #resample if hz not 16k
    if fs_hz != 16000:
        refname = resample(fname)
        #sox $f -r 16k  $output_name
        fs_hz, signal = wav.read(refname)
        signal_length = len(signal)
        
        #remove the resampled file
        os.remove(refname) 
    ...
    if newname in args.test_sentences:
        ...
        plt.subplot(311)
        plt.imshow(feat_powspec.T, origin='lower', aspect='auto', extent=(0,siglen,0,fs_hz/2000))
        #plt.colorbar(c) 
        ...
        plt.imshow(feat_fbank.T, origin='lower', aspect='auto', extent=(0,siglen,0,args.num_filters))
        ...
        plt.subplot(313)
        freq_bins = freq_bins.astype(int)
        plt.imshow(feat_fbank_dB.T, origin='lower', aspect='auto', extent=(0,siglen,0,args.num_filters))
        if args.num_filters == 80:
            plt.yticks(np.arange(0,80,10))
            setlabels=['']
            for h in np.arange(0,80,10)[1:]:
        ...
        plt.savefig('/share/mini1/res/t/asr/studio/read-us/timit/ph_lstm/task/prep_data/melspec_'+str(args.num_filters)+'_'+name+'.png', dpi = 300)
...
#def process(file_lists, ph_dict, dataset,speaker2index, f_h5, args ):
def process(file_lists, lab_lists, dataset,speaker2index, f_h5, args ):
    ...
    for fname in file_lists:
        ...
        if args.feat == 'melspect':
            feat = get_melspectrogram(fname, args)
        else:
            #extract features using HTK
            feat = extract_features(fname, directs[-1], args)    

        if np.max(feat) > max_scale:
            max_scale = np.max(feat)
        if np.min(feat) <min_scale:
            min_scale = np.min(feat)
        ...
        time_info, frame_info, scores_info, ph_seq = extract_labels(lab_lists[directs[-1][:directs[-1].rfind('.')]])  
        
        ##frames to delet
        frend=frame_info[-1][-1]
        frstart=frame_info[0][0]
            
        #"cut off silent frames"
        feat = feat[:frend][:]
        feat = feat[frstart:][:]
        
        frame_info = frame_info - frame_info[0][0]
        time_info = time_info - time_info[0][0]
        
        
        h5_path = dataset+'/'+ fname.split('.')[0].lower().split('/')[-1]
        print(f"h5_path {h5_path} n_frames {feat.shape[0]} phone_segments {len(ph_seq)} speaker {speaker} idx {speaker_idx}",flush=True)
        
        f_h5.create_dataset(f"{h5_path}/feat",data=feat,dtype=np.float32)
        ...
        data_length.append( (h5_path, feat.shape[0])  )
        ...
        for seg, ph_id in enumerate(ph_seq):
            ...
            #datasamples is an item with:
            #[h5 path, number of segment in the utterance, start frame, end frame, phlabel, word, phone label number ]
...
# -----------------------------------------------------------------------------ina-ndpt51714549hmdl_ndpt5101m96d94
# -----------------------------------------------------------------------------
# Main function
def main():
	...
	parser.add_argument('--feat', help='which feature type to use',required=True)
	...
	parser.add_argument('--hconfig',default='../../lib/htk_config/config.feat')
	...
	parser.add_argument('--test_sentences', help='Sentences used for debug',default=['TIM-MHBS0-SI1575_1:0-2611'])
	parser.add_argument('--f', default=False, action='store_true')
	parser.add_argument('--corpus_path',default='/share/mini1/res/t/lid/accentl2/studio-en/l2arctic/aa/co/')
	...
	if not(args.feat == 'melspect'):

		global FEATTYPE
		FEATTYPE = args.feat
		#create a tmp folder for hosting the htk features
		uid = uuid.uuid1()
		global WDIR
		WDIR = os.path.join(os.getcwd(),uid.hex)
		...
		try:
			...
			print("Successfully created temp directory %s " % WDIR)
	...
	if not(args.feat == 'melspect'):
		try:
			...
			print("Error: %s : %s" % (WDIR, e.strerror))
